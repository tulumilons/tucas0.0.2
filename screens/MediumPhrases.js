...
function EasyScreen({ navigation }) {
  ...
  useEffect(() => {
    const getStoredData = async () => {
      try {
        const storedData = await AsyncStorage.getItem("MediumPhrases");
        const storedData2 = await AsyncStorage.getItem(
          "MediumPhrasesTranslatedPortuguese"
        );
        const storedData3 = await AsyncStorage.getItem("MediumAudios");
        if (storedData) {
    ...
    <View style={styles.easyContainer}>
      ...
      <ScrollView style={styles.easyContentScroll}>
        ...
        {EasyPhrases.slice(0, visibleCount).map((phrase, index, arr) => (
          <View key={index} style={styles.mediumListContainer}>
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                setEasyPhrases((prevPhrases) => {
                  ...
                  replaceToArrayAndStore("MediumPhrases", newPhrases);
                ...
                setEasyAudios((prevAudios) => {
                  ...
                  replaceToArrayAndStore("MediumAudios", newAudios);
                  return newAudios;
                });
